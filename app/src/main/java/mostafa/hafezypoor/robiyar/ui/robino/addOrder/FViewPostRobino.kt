...
class FViewPostRobino(val iEvent: IEvent) : Fragment(R.layout.fview_post_robino){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getSettingViewPostRobino(token)
        ...
        btnAddOrder.setOnClickListener {
            if (countOrder.text.toString().isEmpty()){
                ...
                countOrder.error = " حداکثر تعداد سفارش ${maximumOrder} ویو است "
            }else if (countOrder.text.toString().trim().toInt() < minimumOrder){
                countOrder.error = " حداقل تعداد سفارش ${minimumOrder} ویو است "
            } else {
                viewModel.addViewPostRobino(token,postID.text.toString().trim(),countOrder.text.toString().trim())
                viewLifecycleOwner.lifecycleScope.launch {
                    viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED){
