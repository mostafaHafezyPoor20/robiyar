...
class FLikePostRobino (val iEvent: IEvent): Fragment(R.layout.flike_post_robino){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getSettingLike(token)
        ...
        btnAddOrder.setOnClickListener {
            if (countOrder.text.toString().isEmpty()){
                ...
                countOrder.error = " حداکثر تعداد سفارش ${maximumOrder} لایک است "
            }else if (countOrder.text.toString().trim().toInt() < minimumOrder){
                countOrder.error = " حداقل تعداد سفارش ${minimumOrder} لایک است "
            } else {
                viewModel.addLike(token,postID.text.toString().trim(),countOrder.text.toString().trim())
                viewLifecycleOwner.lifecycleScope.launch {
                    viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED){
